...
START_TEST(test_get_inserted)
{
	...
	status = storage_get("table1","key1",&record,test_conn);
	fail_unless(status == 0, "Error getting a record.");

	// read record value
	int fields, int1, int2;
	char str1[MAX_VALUE_LEN];
	fields = sscanf(record.value,"col11 %d, col12 %d, col13 %s",&int1,&int2,str1);
	fail_unless(fields == 3, "Got wrong number of fields.");
	fail_unless(int1 == 10, "Got wrong value.");
	fail_unless(int2 == 20, "Got wrong value.");
	fail_unless(strcmp(trimtrailingspc(str1), "abc") == 0, "Got wrong value.");
}
END_TEST

START_TEST(test_get_nonexisting)
{
	// get a record that does not exist
	struct storage_record record;
	strncpy(record.value, "", sizeof record.value);
	int status = storage_set("table1","random",&record,test_conn);
	fail_unless(status == -1, "Getting a non-existing record should fail.");
}
END_TEST
...
int main(int argc, char *argv[])
{
	...
	// Get test (get an existing record)
	tc = tcase_create("getinserted");
	tcase_set_timeout(tc, TESTTIMEOUT);
	tcase_add_checked_fixture(tc, test_setup_simple, test_teardown);
	tcase_add_test(tc, test_get_inserted);
	suite_add_tcase(s, tc);

	// Get test (get a non-existing record and should fail)
	tc = tcase_create("getnonexisting");
	tcase_set_timeout(tc, TESTTIMEOUT);
	tcase_add_checked_fixture(tc, test_setup_simple, test_teardown);
	tcase_add_test(tc, test_get_nonexisting);
	suite_add_tcase(s, tc);
